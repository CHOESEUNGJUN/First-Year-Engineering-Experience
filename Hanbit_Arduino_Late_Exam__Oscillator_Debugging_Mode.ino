 ...
 *
 * [시리얼 모니터]
 * - 시스템 시작 전: PIR 모션 카운트만 출력 (500ms 간격)
 * - 시스템 동작 중: 온도, 가스, 작동 상태만 출력 (500ms 간격)
 ...
 *
 * [오실로스코프 측정 방법]
 * 1. 기준 신호: CH1에 PIN_LOOP (49번) 연결 권장
 * 2. 측정 신호: CH2에 측정하려는 디버그 핀 연결
 ...
 *
 * [타이밍 최적화]
 * - 시리얼 통신: 115200 baud (기존 19200 대비 6배 빠름)
 * - 타이머 누적 방식: last_xxx += interval (drift 방지)
 * - ATOMIC_BLOCK: 4바이트 volatile 변수 안전 읽기
 * - 시스템 시작 시 타이밍 변수 초기화로 빠른 동작 방지
...
#include <util/atomic.h>   // ATOMIC_BLOCK for safe volatile access
...
volatile unsigned char led_direction_toggle = 0; // LED 순차 점등 방향 (증가/감소)
volatile unsigned char led_danger_toggle = 0;    // LED 위험 모드 점멸용 토글

// ========== 오실로스코프 디버그 핀 토글 상태 ==========
unsigned char pin_led_toggle = 0;                // LED 업데이트 토글 상태
unsigned char pin_rgb_toggle = 0;                // RGB LED 업데이트 토글 상태
unsigned char pin_motor_toggle = 0;              // DC 모터 업데이트 토글 상태
unsigned char pin_sensor_toggle = 0;             // 센서 읽기 토글 상태
unsigned char pin_vibration_toggle = 0;          // 진동 센서 읽기 토글 상태
unsigned char pin_fnd_toggle = 0;                // FND 업데이트 토글 상태
unsigned char pin_melody_toggle = 0;             // 멜로디 음표 재생 토글 상태
unsigned char pin_buzzer_toggle = 0;             // 부저 동작 토글 상태
...
// ========== 시리얼 출력 제어 변수 ==========
unsigned long last_serial_print = 0;             // 시리얼 출력 마지막 시간
...
void loop()
{
  // ========== volatile 변수 안전하게 읽기 (Race Condition 방지) ==========
  unsigned long current_count;
  ATOMIC_BLOCK(ATOMIC_RESTORESTATE) {
    current_count = count_interrupt;
  }

  // ========== Output: 디버그 정보 출력 (500ms 주기로 변경하여 성능 향상) ==========
  if (current_count - last_serial_print >= 500) {
    last_serial_print += 500;  // 누적 오차 방지
    test_print();
  }
  ...
  if (prev_pir_value == 0 && pir_value > 0 && !system_started && !show_start_screen) {
    ...
    if (pir_count >= 10) {
      ...
      start_screen_timer = current_count;  // 타이머 시작
  ...
  if (show_start_screen) {
    ...
    // 3초 경과 시 시스템 시작
    if (current_count - start_screen_timer >= 3000) {
      show_start_screen = false;
      system_started = true;
      lcd.clear();

      // 타이밍 변수 초기화 (시작 직후 빠른 동작 방지)
      last_led_count = current_count;
      last_rgb_count = current_count;
      last_motor_count = current_count;
      last_vibration_count = current_count;
      last_temp_count = current_count;
      last_serial_print = current_count;
      last_melody_count = current_count;
    }
  ...
  // ---------- LED 제어 (500ms 간격) ----------
  // 위험 상태(온도 40도 이상 또는 가스 60% 이상)일 때는 모든 LED 빨간색 점멸
  // 정상 상태일 때는 LED 1번부터 8번까지 순차적으로 켜고, 다시 8번부터 1번까지 끄기
  if (current_count - last_led_count >= 500) {
    last_led_count += 500;  // 누적 오차 방지

    // 오실로스코프 디버그: LED 업데이트 주기 확인
    pin_led_toggle = !pin_led_toggle;
    digitalWrite(PIN_LED, pin_led_toggle);

    // 위험 상태 체크
    bool is_danger = (current_temperature >= 40.0) || (current_gas >= 60);

    if (is_danger) {
      // 위험 상태: 모든 LED 8개 동시에 점멸 (ON/OFF 토글)
      if (led_danger_toggle == 0) {
        for (int i = 1; i <= 8; i++) {
      ...
      if (0 == led_direction_toggle) {
        ...
        if (led_index > 8) {              // 8번 LED 도달 시
          led_index = 8;
          led_direction_toggle = !led_direction_toggle;  // 감소 모드로 전환
        }
      } else {
        led_index--;
        if (led_index < 1) {              // 1번 LED 도달 시
          led_index = 1;
          led_direction_toggle = !led_direction_toggle;  // 증가 모드로 전환
        }
      }
    }
  }
  ...
  if (current_count - last_rgb_count >= 30) {
    last_rgb_count += 30;  // 누적 오차 방지

    // 오실로스코프 디버그: RGB LED 업데이트 주기 확인
  ...
  // 오실로스코프 디버그: FND 업데이트 주기 확인 (매 루프)
  ...
  // ---------- 진동 센서 읽기 (100ms 간격) ----------
  // 피에조 센서로 진동 감지
  if (current_count - last_vibration_count >= 100) {
    last_vibration_count += 100;  // 누적 오차 방지

    // 오실로스코프 디버그: 진동 센서 읽기 주기 확인
  ...
  if (current_count - last_motor_count >= 1000) {
    last_motor_count += 1000;  // 누적 오차 방지

    // 오실로스코프 디버그: DC 모터 업데이트 주기 확인
  ...
  switch(melody_state) {
    case 0:  // 대기 상태 - 새로운 음표 시작
      if (melody_index < noteCount) {
        ...
        if (current_count - last_melody_count >= 3000) {
          melody_loop_count++;            // 멜로디 재생 횟수 증가
          lcd_state = 2;                  // LCD 곡 정보 화면으로 전환
          lcd_timer = current_count;      // LCD 타이머 시작
    ...
    case 1:  // 음표 재생 중 - 지속 시간 체크
      if (current_count - last_melody_count >= durations[melody_index]) {
    ...
    case 2:  // 음표 사이 50ms 간격 대기
      if (current_count - last_melody_count >= 50) {
  ...
  // ---------- 센서 데이터 읽기 (1초 간격) ----------
  // 온도 센서와 가스 센서 값 읽기
  if (current_count - last_temp_count >= 1000) {
    last_temp_count += 1000;  // 누적 오차 방지

    // 오실로스코프 디버그: 온도/가스 센서 읽기 주기 확인
  ...
  // 가스 농도 60% 이상 → 가스 위험 경고 화면 (최우선 순위 2!)
  else if (current_gas >= 60) {
  ...
  // FND가 1000 단위일 때 → 팀 이름 표시 (우선순위 3)
    ...
    if (!flag_fnd_1000) {
      ...
      lcd_timer = current_count;          // 3초 타이머 시작
      flag_fnd_1000 = true;
    }
  } else {
    flag_fnd_1000 = false;
  ...
  if (lcd_state == 5) {
    ...
    // 가스 농도가 60% 미만으로 내려가면 실시간 화면으로 복귀
    if (current_gas < 60) {
      lcd_state = 3;
    }
  } else if (lcd_state != 3 && (current_count - lcd_timer >= LCD_DURATION)) {
    lcd_state = 3;
  }
